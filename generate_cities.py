# -*- coding: utf-8 -*-
"""Generate local SEO city pages for HD הרחקת יונים."""
...
REGIONS = {
    ...
    "הצפון": [
        ...
        ("akko.html", "עכo", "בעכo אנו מתקינים רשתות ודוקרנים לבתים, עסקים ומבנים."),
        ...
        ("tirat-carmel.html", "טירat הכarmel", "בטירat הכarmel אנו מתקינים רשתות ודוקרנים למבני מגורים."),
    ...
    "הדרום": [
        ...
        ("ashdod.html", "אשדod", "באשדod אנו מטפלים בבניינים, מרפסות וגגות עם אחריות מלאה."),
...
<body class="has-sticky-bar">
  <header class="hero page-hero">
    <div class="container">
      <div class="site-header">
        <a href="index.html" class="logo">HD הרחקת יונים</a>
        <nav class="site-nav"><ul><li><a href="index.html#services">שירותים</a></li><li><a href="areas.html">אזורים</a></li><li><a href="index.html#contact-form">צור קשר</a></li></ul></nav>
        <a class="header-cta" href="tel:0528805053">052-880-5053</a>
      </div>
      <div class="page-intro">
        <p class="eyebrow">שירות מקומי ב{city} · {region}</p>
        <h1>הרחקת יונים ב{city}</h1>
        <p class="subtitle">{text}</p>
        <div class="hero-buttons">
          <a class="button primary" href="tel:0528805053">התקשר עכשיו</a>
  ...
  <main>
    <section class="container section">
      <div class="page-content-grid">
        <div>
          <h2>הרחקת יונים ב{city}</h2>
          <p>{text}</p>
          <p>שירותים: <a href="pigeon-spikes.html">דוקרני יונים</a> · <a href="bird-netting.html">רשתות</a> · <a href="electric-bird-deterrent.html">פס חשמלי</a> · <a href="solar-panels-protection.html">הגנה לפאנלים סולאריים</a></p>
          <p><a href="areas.html">← כל האזורים והערים</a></p>
        </div>
        <div class="media-placeholder"><span>📷 תמונה מ{city}<br><small>יתווסף בקרוב</small></span></div>
      </div>
    </section>
    <section class="container section" id="contact-form">
      <h2>הצעת מחיר ב{city}</h2>
      <div class="contact-form-section">
        <form class="lead-form" novalidate>
          <div><label for="name">שם *</label><input id="name" name="name" required></div>
          <div><label for="phone">טלפון *</label><input id="phone" name="phone" type="tel" required></div>
          <div><label for="city">עיר</label><input id="city" name="city" value="{city}"></div>
          <div><label for="message">פרטים</label><textarea id="message" name="message"></textarea></div>
          <button type="submit" class="button primary form-submit">שליחה בוואטסאפ</button>
        </form>
      </div>
    </section>
  </main>
  <footer class="footer"><div class="container"><p>HD הרחקת יונים | <a href="index.html">דף הבית</a> · <a href="areas.html">אזורים</a></p></div></footer>
  ...
  <script type="application/ld+json">{{"@context":"https://schema.org","@type":"LocalBusiness","name":"HD הרחקת יונים","telephone":"+972528805053","areaServed":"{city}","description":"{desc}"}}</script>
...
def slug_to_title(city: str) -> str:
    return f"הרחקת יונים {city} | HD הרחקת יונים"


def meta_desc(city: str) -> str:
    return f"שירות הרחקת יונים מקצועי ב{city}. דוקרנים, רשתות, פס חשמלי והגנה לפאנלים. 18 שנות ניסיון, אחריות 5 שנים."


def render_areas_page(regions: dict) -> str:
    parts = [
        """<!DOCTYPE html>
<html lang="he" dir="rtl">
<head>
  ...
  <meta name="description" content="HD הרחקת יונים – שירות בכל האזורים: גוש דan, השרון, ירושלים, הצפון והדרום. מצאו שירות הרחקת יונים בעיר שלכם.">
...
<body class="has-sticky-bar">
  ...
  <main>"""
    ...
    for region, cities in regions.items():
        parts.append(f'<section class="container section region-block"><h2 class="section-title-bar">{region}</h2><nav class="city-links" aria-label="{region}">')
        for fname, city, _ in cities:
            parts.append(f'<a href="{fname}">הרחקת יונים {city}</a>')
...
def main():
    ...
    urls = ["/", "/areas.html"]
    service_pages = [
        "pigeon-spikes.html", "bird-netting.html",
        "electric-bird-deterrent.html", "solar-panels-protection.html",
    ]
    urls.extend(f"/{p}" for p in service_pages)

    for region, cities in REGIONS.items():
        for fname, city, text in cities:
            title = slug_to_title(city)
            desc = meta_desc(city)
            html = PAGE.format(
                title=title,
                desc=desc,
    ...
    for path in urls:
        ...
        sitemap_lines.append(f"    <loc>{DOMAIN}{path}</loc>")
    ...
    print(f"Generated {count} city pages + areas.html + sitemap.xml")
